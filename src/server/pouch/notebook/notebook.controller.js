...
const find = () => {
  return new Promise((resolve, reject) => {
    NotebookDb.allDocs({
      ...
      .then(result => {
        // console.log('raw resutls ', result);
...
const create = notebook => {
  return new Promise((resolve, reject) => {
    return NotebookDb.post(notebook)
      .then(response => {
        if (!response.ok) {
          reject(response);
        }
    ...
    if (!data._id) {
      ...
      NotebookDb.get(data._id)
        ...
        .then(doc => {
          ...
          data._rev = doc._rev;
          //put updates with id
          NotebookDb.put(data).then(
            response => (!response.ok ? reject(response) : resolve(data)),
          );
        })
        .catch(err => {
          reject(err);
...
export {find, create, remove, update, updateOrCreate};
